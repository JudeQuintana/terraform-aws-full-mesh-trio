locals {
  reachability              = { for this in [var.full_mesh_trio.inspect.reachability] : this => this if var.full_mesh_trio.inspect.reachability }
  diagnostics               = { for this in [var.full_mesh_trio.inspect.diagnostics] : this => this if var.full_mesh_trio.inspect.diagnostics }
  provenance                = { for this in [var.full_mesh_trio.inspect.provenance] : this => this if var.full_mesh_trio.inspect.provenance }
  previous_reachability     = var.full_mesh_trio.inspect.policy_diff.previous_reachability != null
...
resource "local_file" "this_reachability" {
  ...
  filename = format("%s/inspect/%s-reachability.json", path.root, local.full_mesh_trio_name)
}
...
resource "local_file" "this_reachability_simplified" {
  ...
  filename = format("%s/inspect/%s-reachability-simplified.json", path.root, local.full_mesh_trio_name)
}
...
resource "local_file" "this_diagnostics" {
  ...
  filename = format("%s/inspect/%s-diagnostics.json", path.root, local.full_mesh_trio_name)
}
...
resource "local_file" "this_policy_normalization" {
  ...
  filename = format("%s/inspect/%s-policy-normalization.json", path.root, local.full_mesh_trio_name)
}
...
resource "local_file" "this_policy_diff" {
  ...
  filename = format("%s/inspect/%s-policy-diff.json", path.root, local.full_mesh_trio_name)
}
...
resource "local_file" "this_equivalence" {
  ...
  filename = format("%s/inspect/%s-equivalence.json", path.root, local.full_mesh_trio_name)
}
...
resource "local_file" "this_assertions" {
  ...
  filename = format("%s/inspect/%s-assertions.json", path.root, local.full_mesh_trio_name)
}
